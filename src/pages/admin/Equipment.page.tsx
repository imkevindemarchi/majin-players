...
const Equipment: FC = () => {
    ...
    const pageTitle: string = `${id ? "Modifica" : "Nuovo"} Equipaggiamento`;
    ...
    async function getDataHandler() {
        if (id) {
            ...
            const res = await EQUIPMENTS_API.get(id);
            setFormData({
                ...res,
                image: res.id,
            });
    ...
    async function submitHandler(event: MouseEvent<HTMLButtonElement>) {
        ...
        if (isFormValid) {
            if (isEditMode && id) {
                const data: EquipmentT = {
                    id: formData.id,
                    name: formData.name,
                };

                const res = await EQUIPMENTS_API.update(data, id);
                ...
                if (res && typeof res === "string") {
                    if (isImageUpdated && formData.image) {
                        ...
                        if (deleteImageRes) {
                            ...
                            if (imageRes) {
                                ...
                                activateSnackbar(
                                    "Impossibile aggiungere l'immagine all'equipaggiamento",
                                    "error"
                                );
                            }
                        } else {
                    ...
                    activateSnackbar(
                        "Impossibile aggiornare l'equipaggiamento",
                        "error"
                    );
                }
            } else {
                ...
                const res = await EQUIPMENTS_API.create(data);
                if (res && typeof res !== "boolean" && formData.image) {
                    const imageRes = await IMAGES_API.add(res, formData.image);
                    if (imageRes) {
                        activateSnackbar(
                            "Equipaggiamento creato con successo",
                            "success"
                        );
                        navigate(`/admin/equipments/${res}`);
                    } else {
                        activateSnackbar(
                            "Impossibile aggiungere l'immagine all'equipaggiamento",
                            "error"
                    ...
                    activateSnackbar(
                        "Impossibile creare l'equipaggiamento",
                        "error"
                    );
                }
            }
