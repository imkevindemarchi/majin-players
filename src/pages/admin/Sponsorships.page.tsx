...
const Sponsorships: FC = () => {
    ...
    const [tableData, setTableData] = useState<SponsorshipT[]>([]);
    ...
    const [selectedSponsorship, setSelectedSponsorship] =
        useState<SponsorshipT | null>(null);

    const pageTitle: string = "Sponsor";
    ...
    async function getDataHandler() {
        ...
        const sponsorshipsRes = await SPONSORSHIPS_API.getAll(
            from,
            to,
            formData.name
        );
        ...
        if (
            ...
            sponsorshipsRes.data &&
            sponsorshipsRes.totalRecords
        ) {
            setTableData(sponsorshipsRes.data);
            setTotalSponsorships(parseInt(sponsorshipsRes.totalRecords));
        } else
            activateSnackbar(
                "Impossibile recuperare gli sponsor",
                "error"
            );
    ...
    function tableDeleteHandler(rowData: any): void {
        setSelectedSponsorship(rowData);
    ...
    const table = (
        <Table
            ...
            totalRecords={totalSponsorships}
    ...
    async function deleteHandler() {
        ...
        if (selectedSponsorship?.id) {
            const res = await SPONSORSHIPS_API.delete(selectedSponsorship.id);
            ...
            if (res.value) {
                ...
                if (imageRes) {
                    ...
                    activateSnackbar(
                        "Impossibile cancellare l'immagine dello sponsor",
                        "error"
                    );
                }
            } else {
    ...
    const modal = (
        ...
        >
            <span
                ...
                `}
            >
                Confermi di voler eliminare lo sponsor
